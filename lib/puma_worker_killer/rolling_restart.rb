  ...
  class RollingRestart
    ...
    def reap(wait_between_worker_kill = 60) # seconds
      ...
      @cluster.workers.each do |worker, ram|
        @cluster.master.log "#{Time.now} PumaWorkerKiller: Rolling Restart. #{@cluster.workers.count} workers consuming total: #{ get_total_memory } mb. Sending TERM to pid #{worker.pid}."
        worker.term
